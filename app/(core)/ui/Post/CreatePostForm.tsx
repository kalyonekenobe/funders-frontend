...
import { createPost } from '@/app/(core)/actions/post.actions';
import { NotificationType } from '@/app/(core)/utils/notifications.utils';
import { ApplicationRoutes } from '@/app/(core)/utils/routes.utils';
import { PlusIcon, RemoveIcon } from '@/app/(core)/ui/Icons/Icons';

export interface CreatePostFormProps extends FormHTMLAttributes<HTMLFormElement> {
...
const CreatePostForm: FC<CreatePostFormProps> = ({ categories, ...props }) => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const response = await createPost(stateWithoutAttachmentsAndCategories, formData);
    setState(response);
  ...
  useEffect(() => {
    if (!state.isLoaded) {
      setState({
        ...state,
        data: { ...state.data, categories: [], attachments: [] },
        isLoaded: true,
      });
    }
  }, []);
    ...
    <form onSubmit={handleSubmit} {...props}>
      <div className='grid grid-cols-1 w-full gap-3 max-w-5xl'>
        <div className='flex flex-col'>
          ...
          <div className='relative flex w-full rounded overflow-hidden aspect-video border'>
            <Image
              src={
                state.image ||
                process.env.NEXT_PUBLIC_POST_IMAGE_PLACEHOLDER_PATH ||
          ...
          <input
            ...
            onChange={event =>
              setState({
                ...state,
                image: event.target.files?.[0] ? URL.createObjectURL(event.target.files[0]) : null,
              })
            }
          />
        </div>
        <div className='flex flex-col relative gap-3'>
          ...
          <div className='flex flex-col'>
            <label htmlFor='create-post-title' className='text-gray-500 font-medium text-sm mb-1'>
              Title:
            </label>
            <input
              type='text'
          ...
          <div className='flex flex-col'>
            <label htmlFor='create-post-content' className='text-gray-500 font-medium text-sm mb-1'>
              Content:
            </label>
            <textarea
              name='content'
          ...
          <div className='flex flex-col'>
            <label
              htmlFor='create-post-funds-to-be-raised'
            ...
            <input
              ...
              id='create-post-funds-to-be-raised'
              placeholder='100'
          ...
          <div className='flex flex-col'>
            <div className='flex justify-between items-center my-3'>
              ...
              <label
                htmlFor='create-post-attachments'
